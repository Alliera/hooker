...
echo $(find src/ -type f -exec md5sum {} \; | sort -k 2 | md5sum | sed 's/ //g') > checksum
...
if [ $(find build.zip -printf "%s") -gt 300 ] ;then
    ...
    rm build.zip
    rm -rf /build
    sourceguardian/sourceguardian -r -o /build --docker-socket /var/run/sourceguardian-docker.sock --phpversion 7.4 /xircl/src
